...
class TAjaxStubClient extends TAjaxClient
{
	...
	public function render()
	{
		...
		if($this->compareEtags($ETag) == false)
			$this->send($this->renderScripts($this->objects));
	}
...
abstract class TAjaxServer
{
	...
	{
		//only report errors that are set to capture
		if(($errno & ($mask=error_reporting()))!=$errno)
			return;
...
class TAjaxUri
{
	...
	{
		$url = $this->uri;
		if ($pos = strpos($this->uri,'&' . $this->NS))
			$url = substr($this->uri,0,$pos);
		elseif ($pos = strpos($this->uri,'?' . $this->NS))
			$url = substr($this->uri,0,$pos);
		elseif ($pos = strpos($this->uri,$this->NS))
			$url = substr($this->uri,0,$pos);

		$sep = strpos($url, '?') !== false ? '&' : '?';
		return $url.$sep.$this->NS;
	}
...
class TAjaxPathUri extends TAjaxUri
{
	...
	{
		/*
		//from http://www.xisc.com/forum/viewtopic.php?t=3058
		//some tests required before using the following code
		$url = $this->uri;
		if ($pos = strpos($this->uri,'/' . $this->NS))
			$url = substr($this->uri,0,$pos);
		elseif ($pos = strpos($this->uri,'/?' . $this->NS))
